...
(function () {
    ...
    // The practice-site pages set window.GTM_COURSE_MANUAL before this file loads. On
    // those pages the student pastes the container snippet into the HTML by
    // hand, so nothing is auto-loaded here and the Connect box is not shown.
    // The value is the container ID that is already in the file (the course
    // author's), so the status line can tell it apart from the student's.
    var MANUAL = window.GTM_COURSE_MANUAL || false;
    var PREINSTALLED = typeof MANUAL === 'string' ? MANUAL.toUpperCase() : null;
    ...
    if (stored && !MANUAL) { loadContainer(stored); }
    ...
    function updateStatus() {
        ...
        if (window.google_tag_manager) {
            ...
            if (MANUAL) {
                var yours = ids.filter(function (id) { return id !== PREINSTALLED; });
                if (yours.length) {
                    statusDot.className = 'dot live';
                    statusEl.textContent = 'Your container ' + yours.join(', ') + ' is running on this page' +
                        (PREINSTALLED && ids.indexOf(PREINSTALLED) > -1 ? ' (next to the course author\u2019s ' + PREINSTALLED + ')' : '');
                } else {
                    statusDot.className = 'dot off';
                    statusEl.textContent = 'Your container is not on this page yet' +
                        (PREINSTALLED ? ' \u2014 only the course author\u2019s ' + PREINSTALLED + ' is running' : '');
                }
                return;
            }
            statusDot.className = 'dot live';
    ...
    function refreshConnectBox(host) {
        ...
        if (MANUAL || (CONFIG.hideConnectBox && containerId)) {
